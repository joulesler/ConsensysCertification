...
//    payable(items[_sku].buyer).transfer(amountToRefund);
    (bool success, bytes memory data) = payable(items[_sku].buyer).call{value: amountToRefund}("");
  }
  ...
  function addItem(string memory _name, uint _price) public returns (bool) {
    ...
    items[skuCount] = Item({
     ...
     buyer: payable(address(0)) // should not make zero address payable in actual code
    });
